function problem = rascal_cmaes(problem);

%CMAES Optimisation strategy modified for Rascal.

%First pack the parameters into fitting and 
problem = packparams(problem);

% ---------------------- First Handle Input Parameters

%Initial Parameters......
xstart = problem.fitpars(:);                %Initial point
lbounds = problem.fitconstr(:,1);          %Bounds from gui
...
stopTolFun = 0e-12;
stopOnWarnings = 'no'; 
flgWarnOnEqualFunctionValues = 'no';
flgdisplay = 'on';
flgplotting = 'on';
verbosemodulo = '1';
flgscience = 'off';
...
savetime = 15;
popsize = 100;
maxdx = Inf;                            % maximal sensible variable change
mindx = 0;                              % minimal sensible variable change

%Fitting Strategy options....
ParentNumber = floor(popsize / 2);
RecombinationWeights = 1; % 1 ='superlinear decrease', 2 = 'linear', 3 = 'equal';
...
%Update the problem structure with the best fit.
problem.fitpars = bestever.x;
problem.fitconstr = [lbounds(:) ubounds(:)];

fprintf('Final value = %6.4f \n',fmin);
disp(stopflag);

%Unpack the params again
problem = unpackparams(problem);
