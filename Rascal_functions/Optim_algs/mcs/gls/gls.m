...
if nargin==1, 
  addInfoText(' ');
  addInfoText('******** debug mode; old data are used ********')
  addInfoText(' ');
  prt1=func;
  load glsinput;
  prt=prt1;
...
while s<min(5,smax), 
  if nloc==1,
    ...
    if s>3 & monotone & (abest==amin | abest==amax), 
      if prt>1, addInfoText('return since monotone'); end;
      nf=s-sinit;		% number of function values used
      lsdraw; return;
    end;
  else 
...
if nmin==1, 
  ...
  if s==5,  try lsquart; catch; addInfoText('oops'); end; end;	% try quartic interpolation step
  lsdescent;  			% check descent condition 		
  lsconvex; 			% check convexity	
  if convex,
    if prt>1, addInfoText('return since convex'); end;
    nf=s-sinit;			% number of function values used
    lsdraw; return;
  end;
end;
...
while 1,
  lsdraw;
  if prt>1, addInfoText('***** new refinement iteration *****'); end;
  lsdescent;  			% check descent condition 		
  lssat;			% check saturation
  if saturated | s==sold | s>=smax, 
    if saturated & prt>1, addInfoText('return since saturated'); end;
    if s==sold   & prt>1, addInfoText('return since s==sold');   end;
    if s>=smax   & prt>1, addInfoText('return since s>=smax');   end;
    lsdraw;break; 
  end;
