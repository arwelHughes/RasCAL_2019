...
% check box bounds
% if ~isempty(find(sqrt((v)^2)<sqrt((u)^2))) 
...
% if the pre-assigned size of the `large' arrays has already been exceeded, these arrays are made larger
  ...
  if s == smax  % if smax is reached, a new sweep is started 
    ...
    if prt,
      if nsweep == 1
        addInfoText(sprintf('nsw  minl  '));
        if prt > 1
          addInfoText(sprintf('optl    fopt       '))
        end
        addInfoText(sprintf('nf     fbest        xbest\n'))
      end
      minlevel=s;
      addInfoText(sprintf('%3i  %3i',nsweep,minlevel));
      if prt > 1
        addInfoText(sprintf('  %3i',optlevel));
        addInfoText(sprintf('  %10.3e',foptbox));
      end
      addInfoText(sprintf('  %5i  %10.3e',ncall,fbest));
      addInfoText(sprintf('  %10.4f',xbest));
      addInfoText(sprintf('\n'));
    end
    if stop(1) > 1
