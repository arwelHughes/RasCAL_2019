function varargout = differentialevolution(DEParams, paramDefCell, ...
	objFctHandle, objFctSettings, objFctParams, emailParams, optimInfo, rUpdateFn)
...
if nargin == 0 || isempty(DEParams)
	...
	for k=1:length(fieldNames)
		if ~isfield(DEParams, fieldNames{k})
			...
			addInfoText(textwrap2(sprintf(['Warning: Field ''%s'' not included in DEParams. ', ...
...
for parNr = 1:D
	if XVmin(parNr) > XVmax(parNr)
		addInfoText(sprintf('Error: Lower bound (%g) of parameter %s is larger than upper bound (%g).', ...
			XVmin(parNr), getparametername__(parNr, 1), XVmax(parNr)));
		errorFound = true;
	end
	if parGridVector(parNr) < 0
		addInfoText(sprintf('Error: Negative quantization values are not allowed (parameter %s).', ...
			getparametername__(parNr, 1)));
		errorFound = true;
	end		
	minQuantizationUser = 1e-12;
	if parGridVector(parNr) > 0 && parGridVector(parNr) < minQuantizationUser
		addInfoText(sprintf('Error: Minimum quantization step size is %g (parameter %s).', ...
...
if DEParams.displayResults && ~DEParams.saveHistory
	addInfoText(sprintf('Warning: Optimization history can not be displayed if not saved.\n'));
...
if DEParams.useInitParams
		...
		while parNr <= D
			if ~isempty(paramDefCell{parNr, 4})
				...
				if isstruct(objFctParams) && isfield(objFctParams, parameterName) && ...
					...
					if ~isequal(objFctParams.(parameterName), initialMem(index))
						addInfoText(textwrap2(sprintf(['Warning: Initial value of parameter ''%s'' in ', ...
				...
				elseif isnumeric(objFctParams) && ~isempty(objFctParams) && ...
					...
					if isscalar(objFctParams)
						addInfoText(textwrap2(sprintf(['Warning: Initial parameter value ', ...
							'objFctParams was overwritten by settings in paramDefCell.'], ...
							parameterName), textWidth));
					else
						addInfoText(textwrap2(sprintf(['Warning: Initial parameter vector ', ...
							'objFctParams was overwritten by settings in paramDefCell.'], ...
							parameterName), textWidth));
					end
				end
			...
			for d=1:parameterDimVector(parNr)
				...
				if isnan(initialMem(index))
					...
					addInfoText(textwrap2(sprintf(['Warning: No initial value for parameter %s given. ', ...
	...
	elseif isempty(objFctParams)
		...
		addInfoText('Warning: Option DEParams.useInitParams is true but no initial parameter set is given.');
	...
	if checkInitialMem
		...
		for parNr = 1:D
			if ~isnan(initialMem(parNr)) && paramDefCell{parNr,3} > 0 && abs(initialMem(parNr) - quantInitialMem(parNr)) > eps
				addInfoText(textwrap2(sprintf(['Warning: Initial value of parameter %s (%g) ', ...
...
% display title
addInfoText(repmat('*', 1, textWidth)); % ***************
if isstruct(optimInfo) && isfield(optimInfo, 'title')
	addInfoText(textwrap2(sprintf('Starting ''%s'' at %s.', optimInfo.title, ...
		datestr(clock, 'mmm dd, HH:MM')), textWidth));
	if isfield(optimInfo, 'subtitle')
		addInfoText(textwrap2(optimInfo.subtitle));
	end
else
	addInfoText(sprintf('Starting optimization at %s.', datestr(clock, 'mmm dd, HH:MM')));
end

% display number of possible parameter vectors
if isfinite(nOfPossibleMembers)
	addInfoText(sprintf('Number of possible parameter vectors: %g', nOfPossibleMembers));
else
	addInfoText(sprintf('Infinite number of possible parameter vectors.'));
end
...
if (DEParams.maxtime <= 0) || (~isempty(maxclock) && etime(clock, maxclock) > 0)
	addInfoText(repmat('*', 1, textWidth)); % ***************
	addInfoText(textwrap2(sprintf(['Function %s was left because parameter maxtime ', ...
...
timeOver            = false;
while ~timeOver && (iterationNr < DEParams.maxiter)&& (stopFlag==0) && ...
	...
	elseif reinitialization
		...
		% re-initialization
		addInfoText(sprintf('Re-initializing population in iteration %d.', iterationNr));
		if valstddev < DEParams.minvalstddev
			addInfoText(sprintf('Evaluation value std. dev. below threshold (%g < %g).', ...
				valstddev, DEParams.minvalstddev));
		elseif paramstddev < DEParams.minparamstddev
			addInfoText(sprintf('Mean parameter std. dev. below threshold (%g < %g).', ...
				paramstddev, DEParams.minparamstddev));
		elseif noChangeCounter >= DEParams.nochangeiter
			addInfoText(sprintf('Population did not change in last %d iterations.', ...
				DEParams.nochangeiter));
		elseif nofevalCounter > DEParams.nofevaliter
			addInfoText(sprintf('No function evaluations in the last %d iterations.', ...
		...
		for memberNr = 1:NP
			...
			if initialization && memberNr == 1
				...
				displaybestmember__(paramDefCell, parameterDimVector, bestval, bestmem, ...
					allval, initval, initmem, 1, state, optimInfo, sendEmail, playSound, rUpdateFn,objFctSettings);
				...
				if mbtime - startLoopTime > 1
					addInfoText(sprintf('First function evaluation time: %s\n', ...
			...
			% display best member
			displaybestmember__(paramDefCell, parameterDimVector, bestval, bestmem, ...
				allval, initval, initmem, 0, state, optimInfo, sendEmail, playSound, rUpdateFn,objFctSettings);
			...
			if displayMeanEvalTime && nfeval.local == 5 && mbtime - startLoopTime > 1
				addInfoText(sprintf('Mean function evaluation time after %d runs: %s\n', ...
	...
	for memberNr = 1:NP
        ...
        stopFlag = getStopFlag();
        %update the graphs...
        %feval(rUpdateFn,bestmem,objFctSettings);
	...
	if ((DEParams.refreshiter > 0) && (rem(iterationNr, DEParams.refreshiter) == 0)) && ...
		...
		addInfoText(sprintf('Iteration %d finished.', iterationNr));
	end
...
addInfoText(textwrap2(state, textWidth));
...
displaybestmember__(paramDefCell, parameterDimVector, bestval, bestmem, allval, ...
	initval, initmem, 1, state, optimInfo, sendEmail, playSound, rUpdateFn,objFctSettings);
...
if forceResultFileNameDisplay
	addInfoText(sprintf('Results are saved in file %s.', resultFileName));
end
...
function displaybestmember__(paramDefCell, parameterDimVector, bestval, bestmem, ...
	allval, initval, initmem, forceParameterDisplay, state, optimInfo, sendEmail, playSound, rUpdateFn,objFctSettings)
...
if any(bestmem ~= lastbestmem) || forceParameterDisplay
	...
	if bestval == initval && isempty(strfind(state, 'Initial'))
		str = [str(1:end-1), ' (same as initial value)\n'];
	end
...
if sendEmailThisTime
	...
	if forceParameterDisplay
		if ~isempty(strfind(lower(state), 'initial'))
			subject = addInfoText(sprintf('Initial eval. value: %s', formatString), bestval);
		else
			subject = addInfoText(sprintf('Best eval. value: %s', formatString), bestval);
		end
	else
		subject = addInfoText(sprintf('New best eval. value: %s', formatString), bestval);
	end
	if isfield(optimInfo, 'title')
		subject = [subject, addInfoText(' (%s, host %s)', optimInfo.title, gethostname)];
	else
		subject = [subject, addInfoText(' (host %s)', gethostname)];
	end
...
for memberNr = NP:-1:1
	...
	if length(index) > 1
		addInfoText('Warning: More than one equal test vector in allmem (internal error?).');
