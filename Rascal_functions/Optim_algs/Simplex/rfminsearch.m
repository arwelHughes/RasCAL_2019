function [x,fval,exitflag,output] = rfminsearch(funfcn,x,problem,options,varargin)
...
itercount = 0;
func_evals = 0;
fv(:,1) = funfcn(x,problem,func_evals,varargin{:});
func_evals = 1;
how = '';
...
for j = 1:n
    ...
    v(:,j+1) = y;
    x(:) = y; f = funfcn(x,problem,func_evals,varargin{:});
...
while func_evals < maxfun && itercount < maxiter && stopflag == 0
    ...
    x(:) = xr; fxr = funfcn(x,problem,func_evals,varargin{:});
    func_evals = func_evals+1;
    
    if fxr < fv(:,1)
        % Calculate the expansion point
        xe = (1 + rho*chi)*xbar - rho*chi*v(:,end);
        x(:) = xe; fxe = funfcn(x,problem,func_evals,varargin{:});
        func_evals = func_evals+1;
        if fxe < fxr
            v(:,end) = xe;
            fv(:,end) = fxe;
        ...
        if fxr < fv(:,n)
            ...
            if fxr < fv(:,end)
                ...
                x(:) = xc; fxc = funfcn(x,problem,func_evals,varargin{:});
                func_evals = func_evals+1;
                ...
                x(:) = xcc; fxcc = funfcn(x,problem,func_evals,varargin{:});
                func_evals = func_evals+1;
            ...
            if strcmp(how,'shrink')
                for j=two2np1
                    ...
                    x(:) = v(:,j); fv(:,j) = funfcn(x,problem,func_evals,varargin{:});
                end
