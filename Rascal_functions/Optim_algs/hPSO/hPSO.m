function [problem,x] = hPSO(problem)
...
options=hPSOoptions(problem);
nvars = length(problem.fitpars);
...
options = optimset('Display','iter','MaxIter',HybridIter);
...
for i=1:popul*sign(HybridIter)
    [pop(i,:),fxi(i,1)]=fminsearch(@hpso_fun,pop(i,:),options,problem,1);
end
pop=min(pop,ones(popul,1)*space(:,2)');
pop=max(pop,ones(popul,1)*space(:,1)');
fxi=feval(@hpso_fun,pop,problem,1);
...
for i=2:flights
    ...
    for j=1:popul*sign(HybridIter)
        [pop(j,:),fxi(j,1)]=fminsearch(@hpso_fun,pop(i,:),options,problem,1);
        [pnew(j,:),fxipnew(j,1)]=fminsearch(@hpso_fun,pop(i,:),options,problem,1);
    end
    pop=min(pop,ones(popul,1)*space(:,2)');
    pop=max(pop,ones(popul,1)*space(:,1)');
    ...
    fxi=feval(@hpso_fun,pop,problem,1);
    fxipnew=feval(@hpso_fun,pnew,problem,1);
