...
class data_ptr {
  public:
    ...
    T* allocate(SZ _cap) {
        T* t = reinterpret_cast<T*>(emlrtMallocMex(static_cast<size_t>(_cap) * sizeof(T)));
        return t;
    }

    void deallocate(T* _m) {
        if (_m == nullptr || !owner_) {
            return;
        }
        emlrtFreeMex(_m);
    }
    ...
    void reserve(SZ _n, emlrtRTEInfo* _rteInfo = nullptr, emlrtConstCTX _ctx = nullptr) {
        if (_n > capacity_) {
            T* const new_data = allocate(_n);
            if (new_data == nullptr) {
    ...
    void resize(SZ _n, emlrtRTEInfo* _rteInfo = nullptr, emlrtConstCTX _ctx = nullptr) {
        SZ old_size = size_;
        if (_n > old_size) {
      ...
      return static_cast<SZ>(
         emlrtSizeMulR2012b(
           ...
           static_cast<uint32_T>(
             numel_check<N-1>::compute(_rteInfo,_ctx,_size)),
           _rteInfo, _ctx));
    }
};
template<>
...
class array_base {
  ...
  public:
    ...
    template <typename... Dims>
    void set_size(emlrtRTEInfo* _rteInfo, emlrtConstCTX _ctx, Dims... dims) {
        coder::detail::match_dimensions<N == sizeof...(dims)>::check();
        set_size_i<0>(dims...);
    ...
    template <typename... Dims>
    void set_size(SZ dim, Dims... dims) {
        coder::detail::match_dimensions<N == sizeof...(dims)+1>::check();
        set_size_i<0>(dim, dims...);
  ...
  private:
    void ensureCapacity(SZ _newNumel, emlrtRTEInfo* _rteInfo = nullptr, emlrtConstCTX _ctx = nullptr) {
...
class array<char_T, 2> : public array_base<char_T, SizeType, 2> {
  ...
  public:
    ...
    array(char_T* _data, SizeType const* _sz)
        : Base(_data, _sz) {
    }
