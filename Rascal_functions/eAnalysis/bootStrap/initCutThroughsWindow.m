function initCutThroughsWindow(src,ev);

problem = getappdata(0,'problem');
desktop = getappdata(0,'desktop');

window = makeCutThroughswindow(problem);
window.setSize(500,500);
desktop.add(window);
window.setVisible(true);

okButton = window.getOkButton();
set(okButton,'ActionPerformedCallback',@cutsOkButtonPressed)
...
setappdata(0,'cutThroughsWindow',window);
setappdata(0,'cutThroughsChartPanel',chart);
end

%--------------------------------------------------------------------------

function myWindow = makeCutThroughswindow(problem)
myWindow = awtcreate('cutThroguhsWindow');
end

%--------------------------------------------------------------------------

function cutsOkButtonPressed(src,ev)

window = getappdata(0,'cutThroughsWindow');
...
% thisDoubleData = myUtils.convertArray(newData,points);
% cutThroughsGraph.updateTheCurve(thisDoubleData,points-1);
