...
if isempty(s2chain)
  ...
  %lims = [0.0275, 0.18, 0.375, 0.5, 0.625, 0.82, 0.9725];
  %lims = [0.0250    0.1750    0.3750  0.5  0.6250    0.8250    0.9750];
  lims = [0.025 0.05 0.25 0.5 0.75 0.95 0.975];
else
...
for i=1:nbatch
  ...
  for iisample = 1:nsample
    ...
    y   = feval(modelfun,datai,th,i);%varargin{:});
    try
        % Modified to account for changes in length of y
        % Reason for change in y is unclear (AVH 17/1/20)
        if ~isempty(ysave)
            ysave(iisample,:,:) = y(1:size(ysave,2));
        else
            ysave(iisample,:,:) = y;
        end
    catch
